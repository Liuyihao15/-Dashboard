#!/usr/bin/env python3
"""extract_data_v2.py but only outputs JSON (no debug prints)"""
...
for row_xml in rows[1:]:
    ...
    members = float(row.get(10, 0) or 0)  # 注册会员人数
    trade_members = float(row.get(12, 0) or 0)  # 交易会员人数
    gmv = float(row.get(23, 0) or 0)
    orders = float(row.get(18, 0) or 0)
    order_users = float(row.get(22, 0) or 0)  # 广告订单人数
    ...
    if prov is not None:
        ...
        if city_val and city_val not in ('0','NULL','','None','nan','#N/A'):
            if city_val not in city_stats:
                city_stats[city_val] = {'cost': 0, 'members': 0, 'gmv': 0, 'prov': prov}
    ...
    if date_str and len(date_str) >= 8:
        dk = date_str[:8]
        if dk not in daily:
            daily[dk] = {'acq': {'cost':0,'members':0,'gmv':0,'orders':0,'order_users':0,'trade_members':0},
                         'ret': {'cost':0,'members':0,'order_users':0}}
        if '最大化新会员注册' in target:
            daily[dk]['acq']['cost'] += cost
            daily[dk]['acq']['members'] += members
            daily[dk]['acq']['gmv'] += gmv
            daily[dk]['acq']['orders'] += orders
            daily[dk]['acq']['order_users'] += order_users
            daily[dk]['acq']['trade_members'] += trade_members
        else:
            daily[dk]['ret']['cost'] += cost
            daily[dk]['ret']['members'] += members
            daily[dk]['ret']['order_users'] += order_users
            daily[dk]['acq']['trade_members'] += trade_members

dates_sorted = sorted(daily.keys())

out = {'dates': dates_sorted}

# Daily
out['acq_cost'] = [round(daily[d]['acq']['cost'], 2) for d in dates_sorted]
out['acq_members'] = [int(daily[d]['acq']['members']) for d in dates_sorted]
out['acq_gmv'] = [round(daily[d]['acq']['gmv'], 2) for d in dates_sorted]
out['acq_order_users'] = [int(daily[d]['acq']['order_users']) for d in dates_sorted]
out['acq_trade_members'] = [int(daily[d]['acq']['trade_members']) for d in dates_sorted]
out['ret_cost'] = [round(daily[d]['ret']['cost'], 2) for d in dates_sorted]
out['ret_members'] = [int(daily[d]['ret']['members']) for d in dates_sorted]
out['ret_order_users'] = [int(daily[d]['ret']['order_users']) for d in dates_sorted]
...
out['kpi'] = {
    'total_cost': round(total_cost, 2), 'total_members': total_members,
...
with open('/Users/edy/Desktop/看板代码/dashboard_data.json', 'w', encoding='utf-8') as f:
    json.dump(out, f, ensure_ascii=False)
print(f"OK: {len(dates_sorted)} days, {len(prov_names)} provinces, {len(city_names)} cities")
