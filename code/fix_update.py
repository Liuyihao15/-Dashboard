#!/usr/bin/env python3
"""
Fix: Re-generate dashboard_data.json with correct column mapping.
The NEW file format (6/23-6/28) has A-Z 26 columns where:
  B(1)=目标类型, C(2)=日期, F(5)=省份, G(6)=城市, 
  J(9)=消耗, K(10)=注册会员, N(13)=交易会员人数, 
  W(22)=广告订单人数, X(23)=原价交易额, Y(24)=实付交易额

The OLD file (already in JSON) was correctly parsed, but NEW file
mapped trade_members to col 12 instead of col 13.

This script:
1. Restores old JSON data (checkout from git)
2. Re-parses the NEW xlsx with CORRECT column mapping
3. Merges new data correctly
"""
import zipfile, xml.etree.ElementTree as ET, json, os, sys, subprocess
...
# === City to Province map (same as before) ===
CITY_TO_PROVINCE = {
...
def new_parse_xlsx(xlsx_path):
    """
    Parse NEW format xlsx (A-Z columns, 商家粒度的广告会员数据).
    Column mapping:
      B(1)=目标类型, C(2)=日期(YYYYMMDD), F(5)=省份, G(6)=城市
      J(9)=消耗, K(10)=注册会员, N(13)=交易会员人数
    ...
    # Column indices (0-based for sorted dict)
    COL_TARGET_TYPE = 1   # B
    COL_DATE = 2          # C  
    COL_PROVINCE = 5      # F
    COL_CITY = 6          # G
    ...
    for r in rows[1:]:
        ...
        target_type = str(vals.get(COL_TARGET_TYPE, ''))
        ...
        if date_str not in daily:
            daily[date_str] = {
                ...
                'org_gmv':0, 'total_order_users':0
            }
        daily[date_str]['org_gmv'] += org_gmv
        daily[date_str]['total_order_users'] += order_users
        ...
        if province:
            if province not in prov_stats:
                prov_stats[province] = {'cost':0,'members':0,'org_gmv':0,
                                       'order_users':0,'trade_members':0}
...
def merge_data(existing, new_parsed):
    """Merge new parsed data into existing JSON.
    New format: uses correct column indices."""
    ...
    for field, default in acq_daily_fields:
    ...
    for field, default in ret_daily_fields:
        if field not in merged:
            merged[field] = [default] * len(merged['dates'])
        for d in new_dates:
            merged[field].append(new_parsed['daily'].get(d, {}).get(field, default))
    
    # === ret_trade_members (new field from new format) ===
    if 'ret_trade_members' not in merged:
        # Pad to match existing length, then append new data
        merged['ret_trade_members'] = [0] * len(merged['dates'])
    for d in new_dates:
        val = new_parsed['daily'].get(d, {}).get('ret_trade_members', 0)
        merged['ret_trade_members'].append(val)
    
    # === org_gmv ===
    if 'org_gmv' not in merged:
        merged['org_gmv'] = [0] * len(merged['dates'])
    for d in new_dates:
        merged['org_gmv'].append(new_parsed['daily'].get(d, {}).get('org_gmv', 0))
    
    # === acq_gmv (保留原有值，新数据填充0) ===
    if 'acq_gmv' not in merged:
        merged['acq_gmv'] = [0] * len(merged['dates'])
    for d in new_dates:
        merged['acq_gmv'].append(0)  # new format doesn't have this breakdown
    
    # === total_order_users (new field) ===
    if 'total_order_users' not in merged:
        merged['total_order_users'] = [0] * len(merged['dates'])
    for d in new_dates:
        val = new_parsed['daily'].get(d, {}).get('total_order_users', 0)
        merged['total_order_users'].append(val)
    ...
    prov_fields = ['prov_cost','prov_members','prov_org_gmv',
                   'prov_order_users','prov_trade_members']
...
def main():
    # Step 1: Restore old JSON from git
    print("🔙 恢复旧JSON数据...")
    os.chdir(DASHBOARD_DIR)
    subprocess.run(['git', 'checkout', '49fc8b2', '--', 'dashboard_data.json'], 
                   capture_output=True)
    ...
    # Step 2: Parse new xlsx with CORRECT column mapping
    print(f'\n📂 解析新文件 (修正列映射)...')
    new_parsed = new_parse_xlsx(XLSX_PATH)
    print(f'   日期: {new_parsed["dates"]}')
    print(f'   省份: {len(new_parsed["prov_stats"])}个')
    print(f'   城市: {len(new_parsed["city_stats"])}个')
    
    # Verify: sum trade_members from new data
    total_trade = 0
    for d in new_parsed['dates']:
        acq_t = new_parsed['daily'][d].get('acq_trade_members', 0)
        ret_t = new_parsed['daily'][d].get('ret_trade_members', 0)
        total_trade += acq_t + ret_t
    print(f'   新数据交易会员人数合计: {total_trade}')
    ...
    print(f'\n=== 合并后验证 ===')
    new_start_idx = len(existing['dates'])
    total_trade_new = sum(merged.get('acq_trade_members', [])[new_start_idx:]) + sum(merged.get('ret_trade_members', [])[new_start_idx:])
    print(f'   新追加交易会员人数: {total_trade_new}')
    
    total_acq_cost_new = sum(merged.get('acq_cost', [0])[new_start_idx:])
    total_ret_cost_new = sum(merged.get('ret_cost', [0])[new_start_idx:])
    print(f'   新追加消耗: acq={total_acq_cost_new:.2f}, ret={total_ret_cost_new:.2f}')
    
    total_acq_members_new = sum(merged.get('acq_members', [0])[new_start_idx:])
    total_ret_members_new = sum(merged.get('ret_members', [0])[new_start_idx:])
    print(f'   新追加注册会员: {total_acq_members_new}')
    
    total_org_gmv_new = sum(merged.get('org_gmv', [0])[new_start_idx:])
    print(f'   新追加原价GMV: {total_org_gmv_new:.2f}')
    ...
    print(f'   交易会员(拉新): {sum(merged.get("acq_trade_members",[0])):,.0f}')
    print(f'   交易会员(老客): {sum(merged.get("ret_trade_members",[0])):,.0f}')
    ...
    git_result = subprocess.run(
        ['bash', '-c', 'cd /Users/edy/Desktop/霸王茶姬看板 && GIT_SSH_COMMAND="ssh -i ~/.ssh/github_dashboard" git add -A && GIT_SSH_COMMAND="ssh -i ~/.ssh/github_dashboard" git commit -m "fix: correct column mapping for 6/23-6/28 trade_members & ROI" && GIT_SSH_COMMAND="ssh -i ~/.ssh/github_dashboard" git push origin gh-pages'],
