...
# 列: 1=TARGET, 2=日期, 5=城市, 9=消耗, 10=注册会员, 22=广告订单人数
total_ou_all = 0
total_ou_filtered = 0  # 被 save_data 筛掉的
filtered_rows = []  # 记录筛掉的行信息
...
for row_xml in rows[1:]:
    ...
    if not prov:
        total_ou_filtered += ou
        filtered_rows.append(f'  城市="{city_val}", OU={int(ou)}, TARGET="{row.get(1,"")}"')

print(f'所有行广告订单人数合计: {int(total_ou_all):,}')
print(f'被省市区映射筛掉的: {int(total_ou_filtered):,}')
print(f'\n被筛掉的行明细（前30）:')
for r in filtered_rows[:30]:
    print(r)
if len(filtered_rows) > 30:
    print(f'  ... 共 {len(filtered_rows)} 行被筛掉')
